...
def get_least_busy(jira, members):
    ...
    for member in members:
        try:
            ...
            ticket = jira.search_issues(f'assignee = "{member["id"]}" AND status != closed AND status != resolved')
...
def search_unassigned_issue(key, jira):
    try:
        #Returns all tickets that has no assigned member and not done
        return jira.search_issues(f'project={key} AND assignee is EMPTY AND status != closed AND status !=resolved')
    except Exception as e:
        logging.error(f"Error gathering unassigned issues: {e}")

def close_resolved_tickets_auto(jira):
    try:
        #Jquery to check if the ticket is resolved or not
        jquery = f'project={proj_key} AND status ="Resolved"'
        resolved_ticket = jira.search_issues(jquery)

        for ticket in resolved_ticket:
            #Checks if ticket has a resolved status
            if ticket.fields.status.name.lower() == "resolved":
            ...
            if datetime.now() - resolved_date > timedelta(days=2):
                ...
                if close_ticket:
                    ...
                    body = (
                        ...
                        f"{jira_connect.server_url}/browse/{ticket.key}"
                    )

                    send_email(reciever_email, subject, body)
                    #Moves ticket from one status to done status and closes ticket
                    jira.transition_issue(ticket, close_ticket['id'])
                    print(f"Ticket Closed ({ticket.key})")
    except Exception as e:
...
try:
    ticket_amount = jira_connect.search_issues(f'project={proj_key}')
    tickets_unassigned_amount = search_unassigned_issue(proj_key, jira_connect)
    logging.info(f"Ticket Amount: {len(ticket_amount)}, Unassigned Ticket Amount: {len(tickets_unassigned_amount)}")

    #Check if no tickets available if there is assign least busy worker
    if len(tickets_unassigned_amount) == 0:
        logging.info("No unassinged tickets found.")
    else:        
        for ticket in tickets_unassigned_amount:
            try:
